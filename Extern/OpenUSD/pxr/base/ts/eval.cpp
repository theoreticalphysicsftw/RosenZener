    ...
    const double d)
{
    ...
    if (discrim < 0)
    {
        ...
        const double phi = std::acos(Clamp(t, -1, 1));
    ...
    const _Cubic &cubic)
{
    ...
    const bool aZero = IsClose(cubic.a, 0, epsilon);
    const bool bZero = IsClose(cubic.b, 0, epsilon);
    const bool cZero = IsClose(cubic.c, 0, epsilon);
      ...
      _location(location)
{
    ...
    if (_haveInnerLoops)
    {
        const Interval loopedInterval = _data->loopParams.GetLoopedInterval();
...
void _LoopResolver::_ResolveInner()
{
    ...
    const Interval loopedInterval = lp.GetLoopedInterval();
    const Interval protoInterval = lp.GetPrototypeInterval();
