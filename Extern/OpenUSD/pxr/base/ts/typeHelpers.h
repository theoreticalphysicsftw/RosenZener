...
template <>
struct Ts_IsSupportedValueType<Half> :
    public std::bool_constant<true> {};
...
// Half doesn't have an overload for std::isfinite, so we provide an adapter.
//
template <typename T>
bool Ts_IsFinite(T value);

template <>
TS_API bool Ts_IsFinite(const Half value);
