// Modified from the original OpenUSD file.
// Copyright 2024 Mihail Mladenov

//
// Copyright 2016 Pixar
//
// Licensed under the terms set forth in the LICENSE.txt file available at
// https://openusd.org/license.
//
////////////////////////////////////////////////////////////////////////
// This file is generated by a script.  Do not edit directly.  Edit the
// vec.template.h file to make changes.

#ifndef PXR_BASE_GF_VEC3I_H
#define PXR_BASE_GF_VEC3I_H

/// \file gf/vec3i.h
/// \ingroup group_gf_LinearAlgebra

#include "pxr/pxr.h"
#include "pxr/base/tf/diagnostic.h"
#include "pxr/base/gf/api.h"
#include "pxr/base/gf/limits.h"
#include "pxr/base/gf/traits.h"
#include "pxr/base/tf/hash.h"

#include <cstddef>

#include <iosfwd>

PXR_NAMESPACE_OPEN_SCOPE

class Vec3i;

template <>
struct IsVec<class Vec3i> { static const bool value = true; };

/// \class Vec3i
/// \ingroup group_gf_LinearAlgebra
///
/// Basic type for a vector of 3 int components.
///
/// Represents a vector of 3 components of type \c int.
/// It is intended to be fast and simple.
///
class Vec3i
{
public:
    /// Scalar element type and dimension.
    typedef int ScalarType;
    static const size_t dimension = 3;

    /// Default constructor does no initialization.
    Vec3i() = default;

    /// Initialize all elements to a single value.
    constexpr explicit Vec3i(int value)
        : _data{ value, value, value }
    {
    }

    /// Initialize all elements with explicit arguments.
    constexpr Vec3i(int s0, int s1, int s2)
        : _data{ s0, s1, s2 }
    {
    }

    /// Construct with pointer to values.
    template <class Scl>
    constexpr explicit Vec3i(Scl const *p)
        : _data{ p[0], p[1], p[2] }
    {
    }
 
    /// Create a unit vector along the X-axis.
    static Vec3i XAxis() {
        Vec3i result(0);
        result[0] = 1;
        return result;
    }
    /// Create a unit vector along the Y-axis.
    static Vec3i YAxis() {
        Vec3i result(0);
        result[1] = 1;
        return result;
    }
    /// Create a unit vector along the Z-axis.
    static Vec3i ZAxis() {
        Vec3i result(0);
        result[2] = 1;
        return result;
    }

    /// Create a unit vector along the i-th axis, zero-based.  Return the zero
    /// vector if \p i is greater than or equal to 3.
    static Vec3i Axis(size_t i) {
        Vec3i result(0);
        if (i < 3)
            result[i] = 1;
        return result;
    }

    /// Set all elements with passed arguments.
    Vec3i &Set(int s0, int s1, int s2) {
        _data[0] = s0;
        _data[1] = s1;
        _data[2] = s2;
        return *this;
    }

    /// Set all elements with a pointer to data.
    Vec3i &Set(int const *a) {
        return Set(a[0], a[1], a[2]);
    }

    /// Direct data access.
    int const *data() const { return _data; }
    int *data() { return _data; }
    int const *GetArray() const { return data(); }

    /// Indexing.
    int const &operator[](size_t i) const { return _data[i]; }
    int &operator[](size_t i) { return _data[i]; }

    /// Hash.
    friend inline size_t hash_value(Vec3i const &vec) {
        return TfHash::Combine(vec[0], vec[1], vec[2]);
    }

    /// Equality comparison.
    bool operator==(Vec3i const &other) const {
        return _data[0] == other[0] &&
               _data[1] == other[1] &&
               _data[2] == other[2];
    }
    bool operator!=(Vec3i const &other) const {
        return !(*this == other);
    }

    // TODO Add inequality for other vec types...
    /// Equality comparison.
    GF_API
    bool operator==(class Vec3d const &other) const;
    /// Equality comparison.
    GF_API
    bool operator==(class Vec3f const &other) const;
    /// Equality comparison.
    GF_API
    bool operator==(class Vec3h const &other) const;
    
    /// Create a vec with negated elements.
    Vec3i operator-() const {
        return Vec3i(-_data[0], -_data[1], -_data[2]);
    }

    /// Addition.
    Vec3i &operator+=(Vec3i const &other) {
        _data[0] += other[0];
        _data[1] += other[1];
        _data[2] += other[2];
        return *this;
    }
    friend Vec3i operator+(Vec3i const &l, Vec3i const &r) {
        return Vec3i(l) += r;
    }

    /// Subtraction.
    Vec3i &operator-=(Vec3i const &other) {
        _data[0] -= other[0];
        _data[1] -= other[1];
        _data[2] -= other[2];
        return *this;
    }
    friend Vec3i operator-(Vec3i const &l, Vec3i const &r) {
        return Vec3i(l) -= r;
    }

    /// Multiplication by scalar.
    Vec3i &operator*=(double s) {
        _data[0] *= s;
        _data[1] *= s;
        _data[2] *= s;
        return *this;
    }
    Vec3i operator*(double s) const {
        return Vec3i(*this) *= s;
    }
    friend Vec3i operator*(double s, Vec3i const &v) {
        return v * s;
    }

        /// Division by scalar.
    Vec3i &operator/=(int s) {
        _data[0] /= s;
        _data[1] /= s;
        _data[2] /= s;
        return *this;
    }
    Vec3i operator/(int s) const {
        return Vec3i(*this) /= s;
    }
    
    /// See Dot().
    int operator*(Vec3i const &v) const {
        return _data[0] * v[0] + _data[1] * v[1] + _data[2] * v[2];
    }

    /// Returns the projection of \p this onto \p v. That is:
    /// \code
    /// v * (*this * v)
    /// \endcode
    Vec3i GetProjection(Vec3i const &v) const {
        return v * (*this * v);
    }

    /// Returns the orthogonal complement of \p this->GetProjection(b).
    /// That is:
    /// \code
    ///  *this - this->GetProjection(b)
    /// \endcode
    Vec3i GetComplement(Vec3i const &b) const {
        return *this - this->GetProjection(b);
    }

    /// Squared length.
    int GetLengthSq() const {
        return *this * *this;
    }

 
private:
    int _data[3];
};

/// Output a Vec3i.
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream& operator<<(std::ostream &, Vec3i const &);


/// Returns component-wise multiplication of vectors \p v1 and \p v2.
inline Vec3i
CompMult(Vec3i const &v1, Vec3i const &v2) {
    return Vec3i(
        v1[0] * v2[0],
        v1[1] * v2[1],
        v1[2] * v2[2]
        );
}

/// Returns component-wise quotient of vectors \p v1 and \p v2.
inline Vec3i
CompDiv(Vec3i const &v1, Vec3i const &v2) {
    return Vec3i(
        v1[0] / v2[0],
        v1[1] / v2[1],
        v1[2] / v2[2]
        );
}

/// Returns the dot (inner) product of two vectors.
inline int
Dot(Vec3i const &v1, Vec3i const &v2) {
    return v1 * v2;
}

 
PXR_NAMESPACE_CLOSE_SCOPE

#endif // PXR_BASE_GF_VEC3I_H
