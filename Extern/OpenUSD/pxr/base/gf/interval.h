...
public:
    ...
    friend inline size_t hash_value(Interval const &i) {
    ...
    double GetSize() const {
        return Max( 0.0, _max.value - _min.value );
    }
    ...
    Interval operator-() const {
        return Interval(-_max.value, -_min.value, _max.closed, _min.closed);
    }
    ...
    static Interval GetFullInterval() {
...
/// Output a Interval using the format (x, y).
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream &operator<<(std::ostream&, const Interval&);
