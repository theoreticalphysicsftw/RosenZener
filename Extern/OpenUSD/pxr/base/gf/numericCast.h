...
constexpr bool
IntegerCompareLess(T t, U u) noexcept
{
...
enum NumericCastFailureType {
    NumericCastPosOverflow,  ///< Value too high to convert.
    NumericCastNegOverflow,  ///< Value too low to convert.
    NumericCastNaN           ///< Value is a floating-point NaN.
};

/// Attempt to convert \p from to a value of type \p To "safely".  From and To
/// must be arithmetic types according to IsArithmetic -- either integral or
/// floating-point types (including Half).  Return a std::optional holding the
...
/// Half), then \p from can safely convert to To if it is neither a NaN nor an
...
/// Half's _finite_ range will produce a +/- inf Half.
///
template <class To, class From>
std::optional<To>
NumericCast(From from, NumericCastFailureType *failType = nullptr)
{
    static_assert(IsArithmetic<From>::value &&
                  IsArithmetic<To>::value);

    using FromLimits = std::numeric_limits<From>;
    using ToLimits = std::numeric_limits<To>;

    auto setFail = [&failType](NumericCastFailureType ft) {
        if (failType) {
            *failType = ft;
        };
    };
    ...
    if constexpr (std::is_integral_v<From> &&
        ...
        if (IntegerCompareLess(from, ToLimits::min())) {
            setFail(NumericCastNegOverflow);
            return {};
        }
        if (IntegerCompareLess(ToLimits::max(), from)) {
            setFail(NumericCastPosOverflow);
            return {};
        }
        // In-range.
    ...
    // float -> int.
        ...
        // works okay for Half whose max is ~65,000 when converting to
