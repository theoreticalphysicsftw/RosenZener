...
class Plane
{
  public:
    ...
    Plane(const Vec3d &normal, double distanceToOrigin) {
    ...
    /// length first.
    void                Set(const Vec3d &normal, double distanceToOrigin) {
    ...
    /// plane.
    GF_API
    bool IntersectsPositiveHalfSpace(const Range3d &box) const;

    /// Returns true if the given point is on the plane or within its positive
    /// half space.
    bool IntersectsPositiveHalfSpace(const Vec3d &pt) const {
...
/// plane's positive and negative half-spaces; you can use Plane::Reorient()
...
GF_API
bool FitPlaneToPoints(const std::vector<Vec3d>& points, Plane* fitPlane);

/// Output a Plane using the format [(nx ny nz) distance].
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream& operator<<(std::ostream&, const Plane&);
