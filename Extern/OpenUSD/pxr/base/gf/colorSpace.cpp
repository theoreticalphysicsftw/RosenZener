...
TF_REGISTRY_FUNCTION(TfType) {
    TfType::Define<ColorSpace>();
}

TF_DEFINE_PUBLIC_TOKENS(ColorSpaceNames, GF_COLORSPACE_NAME_TOKENS);

bool ColorSpace::IsValid(const TfToken& name) {
...
ColorSpace::ColorSpace(const TfToken& name)
: _data(new _Data())
{
...
ColorSpace::ColorSpace(const TfToken& name,
                           const Vec2f &redChroma,
                           const Vec2f &greenChroma,
                           const Vec2f &blueChroma,
                           const Vec2f &whitePoint,
                           float gamma,
                           float linearBias)
: _data(new _Data())
{
...
bool ColorSpace::operator==(const ColorSpace &lh) const
{
...
void ColorSpace::ConvertRGBSpan(const ColorSpace& to, TfSpan<float> rgb) const
{
...
void ColorSpace::ConvertRGBASpan(const ColorSpace& to, TfSpan<float> rgba) const
{
...
Color ColorSpace::Convert(const ColorSpace& srcColorSpace, const Vec3f& rgb) const
{
    Color c(rgb, srcColorSpace);
    return Color(c, *this);
}

TfToken ColorSpace::GetName() const
{
    NcColorSpaceM33Descriptor desc;
...
Matrix3f ColorSpace::GetRGBToXYZ() const
{
    NcColorSpaceM33Descriptor desc;
    if (!NcGetColorSpaceM33Descriptor(_data->colorSpace, &desc)) {
    ...
    return Matrix3f(m[0], m[1], m[2],
                      m[3], m[4], m[5],
                      m[6], m[7], m[8]);
}

float ColorSpace::GetLinearBias() const
{
    NcColorSpaceM33Descriptor desc;
    if (!NcGetColorSpaceM33Descriptor(_data->colorSpace, &desc)) {
        return 0.0f;
...
float ColorSpace::GetGamma() const
{
    NcColorSpaceM33Descriptor desc;
    if (!NcGetColorSpaceM33Descriptor(_data->colorSpace, &desc)) {
        return 1.0f;
...
std::pair<float, float> ColorSpace::GetTransferFunctionParams() const
{
    float K0, phi;
...
std::tuple<Vec2f, Vec2f, Vec2f, Vec2f>
    ColorSpace::GetPrimariesAndWhitePoint() const
{
    NcColorSpaceDescriptor desc;
    if (!NcGetColorSpaceDescriptor(_data->colorSpace, &desc)) {
        return std::make_tuple(Vec2f(0.0f), Vec2f(0.0f), Vec2f(0.0f), Vec2f(0.0f));
    }
    return std::make_tuple(Vec2f(desc.redPrimary.x, desc.redPrimary.y),
                           Vec2f(desc.greenPrimary.x, desc.greenPrimary.y),
                           Vec2f(desc.bluePrimary.x, desc.bluePrimary.y),
                           Vec2f(desc.whitePoint.x, desc.whitePoint.y));
}
