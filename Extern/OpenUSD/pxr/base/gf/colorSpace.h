...
/// \ingroup group_gf_Color
///
/// Color spaces natively supported by  to define scene referred color values.
...
TF_DECLARE_PUBLIC_TOKENS(ColorSpaceNames, GF_API, 
...
///
/// The color spaces natively recognized by ColorSpace are listed in 
/// ColorSpaceNames.

class ColorSpace {
    friend class Color;
public:    
    /// Construct a ColorSpace from a name token.
    ///
    /// \param name The name token of the color space.
    GF_API 
    explicit ColorSpace(const TfToken& name);

    /// Check if a color space name is valid for constructing
    /// a ColorSpace by name.
    GF_API
    ...
    GF_API 
    bool operator ==(const ColorSpace &rh) const;
    ...
    bool operator !=(const ColorSpace &rh) const { return !(*this == rh); }
    ...
    GF_API 
    void ConvertRGBSpan(const ColorSpace& srcColorSpace, TfSpan<float> rgb) const;
    ...
    GF_API 
    void ConvertRGBASpan(const ColorSpace& srcColorSpace, TfSpan<float> rgba) const;

    /// Convert a rgb triplet in a certain color space to "this" color space.
    GF_API
    Color Convert(const ColorSpace& srcColorSpace, const Vec3f& rgb) const;

    /// Get the RGB to XYZ conversion matrix.
    ///
    /// \return The RGB to XYZ conversion matrix.
    GF_API 
    Matrix3f GetRGBToXYZ() const;
