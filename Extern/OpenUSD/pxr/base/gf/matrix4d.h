...
template <>
struct IsMatrix<class Matrix4d> { static const bool value = true; };
...
///
/// The following methods interpret a Matrix4d as a 3D
...
public:
    ...
    ///
    GF_API
    explicit Matrix4d(const std::vector< std::vector<double> >& v);
    ...
    ///
    GF_API
    explicit Matrix4d(const std::vector< std::vector<float> >& v);
    ...
    ///
    GF_API
    explicit Matrix4d(const std::vector<double>& r0,
    ...
    ///
    GF_API
    explicit Matrix4d(const std::vector<float>& r0,
    ...
    GF_API
    Matrix4d(const Rotation& rotate,
               const Vec3d& translate);

    /// Constructor. Initializes a transformation matrix to perform the
    /// indicated rotation and translation.
    GF_API
    Matrix4d(const Matrix3d& rotmx,
               const Vec3d& translate);
    /// This explicit constructor converts a "float" matrix to a "double" matrix.
    GF_API
    explicit Matrix4d(const class Matrix4f& m);

    /// Sets a row of the matrix from a Vec4.
    void SetRow(int i, const Vec4d & v) {
        _mtx[i][0] = v[0];
        _mtx[i][1] = v[1];
        _mtx[i][2] = v[2];
        _mtx[i][3] = v[3];
    ...
    void SetColumn(int i, const Vec4d & v) {
        _mtx[0][i] = v[0];
        _mtx[1][i] = v[1];
        _mtx[2][i] = v[2];
        _mtx[3][i] = v[3];
    ...
    Vec4d GetRow(int i) const {
        return Vec4d(_mtx[i][0], _mtx[i][1], _mtx[i][2], _mtx[i][3]);
    }

    /// Gets a column of the matrix as a Vec4.
    Vec4d GetColumn(int i) const {
        return Vec4d(_mtx[0][i], _mtx[1][i], _mtx[2][i], _mtx[3][i]);
    }
    ...
    Matrix4d& Set(double m00, double m01, double m02, double m03, 
                    double m10, double m11, double m12, double m13, 
                    double m20, double m21, double m22, double m23, 
                    double m30, double m31, double m32, double m33) {
    ...
    Matrix4d& Set(const double m[4][4]) {
        _mtx[0][0] = m[0][0];
        _mtx[0][1] = m[0][1];
        _mtx[0][2] = m[0][2];
        _mtx[0][3] = m[0][3];
    ...
    GF_API
    Matrix4d& SetDiagonal(const Vec4d&);
    ...
    /// Hash.
    friend inline size_t hash_value(Matrix4d const &m) {
    ...
    GF_API
    Matrix4d GetInverse(double* det = NULL, double eps = 0) const;
    ...
    void SetRow3(int i, const Vec3d & v) {
        _mtx[i][0] = v[0];
        _mtx[i][1] = v[1];
        _mtx[i][2] = v[2];
    }
    ...
    bool HasOrthogonalRows3() const {
        ...
        //     Rotation::Decompose() could use).
        Vec3d axis0(GetRow3(0)), axis1(GetRow3(1)), axis2(GetRow3(2));
        return (Abs(Dot(axis0, axis1)) < GF_MIN_ORTHO_TOLERANCE && 
                Abs(Dot(axis0, axis2)) < GF_MIN_ORTHO_TOLERANCE && 
                Abs(Dot(axis1, axis2)) < GF_MIN_ORTHO_TOLERANCE);
    }
    ...
    GF_API
    Matrix4d GetOrthonormalized(bool issueWarning=true) const;
    ...
    friend Matrix4d operator *(const Matrix4d& m1, double d)
    {
        Matrix4d m = m1;
        return m *= d;
    }
    ...
    ///
    // Returns the product of a matrix and a double.
    friend Matrix4d operator *(double d, const Matrix4d& m)
    {
        return m * d;
    }
    ...
    GF_API
    friend Matrix4d operator -(const Matrix4d& m);
    ...
    friend inline Vec4d operator *(const Matrix4d& m, const Vec4d& vec) {
        return Vec4d(vec[0] * m._mtx[0][0] + vec[1] * m._mtx[0][1] + vec[2] * m._mtx[0][2] + vec[3] * m._mtx[0][3],
    ...
    friend inline Vec4d operator *(const Vec4d &vec, const Matrix4d& m) {
        return Vec4d(vec[0] * m._mtx[0][0] + vec[1] * m._mtx[1][0] + vec[2] * m._mtx[2][0] + vec[3] * m._mtx[3][0],
    ...
    GF_API
    Matrix4d& SetScale(double scaleFactor);
    ...
    /// translation by \e translate.
    GF_API
    Matrix4d& SetTransform(const Matrix3d& rotmx,
                             const Vec3d& translate);
    ...
    /// this method.
    GF_API
    Quatd  ExtractRotationQuat() const;
    ...
    /// this method.
    GF_API
    Matrix3d ExtractRotationMatrix() const;
    ...
    /// (0, 0, 0, 1)).
    Vec3d TransformAffine(const Vec3d &vec) const {
        return Vec3d(
            vec[0] * _mtx[0][0] + vec[1] * _mtx[1][0] + vec[2] * _mtx[2][0] + _mtx[3][0],
            vec[0] * _mtx[0][1] + vec[1] * _mtx[1][1] + vec[2] * _mtx[2][1] + _mtx[3][1],
            vec[0] * _mtx[0][2] + vec[1] * _mtx[1][2] + vec[2] * _mtx[2][2] + _mtx[3][2]);
...
private:
    ...
    void _Jacobi3(Vec3d *eigenvalues, Vec3d eigenvectors[3]) const;
...
GF_API 
bool IsClose(Matrix4d const &m1, Matrix4d const &m2, double tolerance);

/// Output a Matrix4d
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream& operator<<(std::ostream &, Matrix4d const &);
