...
std::ostream& 
operator<<(std::ostream &out, Vec3h const &v)
{
    return out << '(' 
        << _OstreamHelperP(v[0]) << ", " 
        << _OstreamHelperP(v[1]) << ", " 
...
bool
Vec3h::operator==(Vec3d const &other) const
{
...
bool
Vec3h::operator==(Vec3f const &other) const
{
    return _data[0] == other[0] &&
           _data[1] == other[1] &&
           _data[2] == other[2];
...
bool
Vec3h::operator==(Vec3i const &other) const
{
    return _data[0] == other[0] &&
           _data[1] == other[1] &&
           _data[2] == other[2];
...
void
Vec3h::BuildOrthonormalFrame(
    Vec3h *v1, Vec3h *v2, Half eps) const
{
...
{
    Vec3h ax,bx,cx,ay,by,cy,az,bz,cz;
                        ...
                        Vec3h* v2, Half eps)
{
	...
	*v1 = *v2 = Vec3h(0);
	...
	Vec3h unitDir = v0 / len;
	*v1 = Vec3h::XAxis() ^ unitDir;

	if (Sqr(*v1) < Sqr(1e-4))
	    *v1 = Vec3h::YAxis() ^ unitDir;
...
Vec3h
Slerp(double alpha, const Vec3h &v0, const Vec3h &v1)
{
    ...
    if ( fabs(sinAngle) < 0.00001 ) {
        ...
        Vec3h v = v0 * cos(alpha*M_PI) + vX * sin(alpha*M_PI);
        return v;
    }
