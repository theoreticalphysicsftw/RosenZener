...
template <>
struct IsMatrix<class Matrix2d> { static const bool value = true; };
...
public:
    ...
    ///
    GF_API
    explicit Matrix2d(const std::vector< std::vector<double> >& v);
    ...
    ///
    GF_API
    explicit Matrix2d(const std::vector< std::vector<float> >& v);

    /// This explicit constructor converts a "float" matrix to a "double" matrix.
    GF_API
    explicit Matrix2d(const class Matrix2f& m);

    /// Sets a row of the matrix from a Vec2.
    void SetRow(int i, const Vec2d & v) {
        _mtx[i][0] = v[0];
        _mtx[i][1] = v[1];
    }

    /// Sets a column of the matrix from a Vec2.
    void SetColumn(int i, const Vec2d & v) {
        _mtx[0][i] = v[0];
        _mtx[1][i] = v[1];
    }

    /// Gets a row of the matrix as a Vec2.
    Vec2d GetRow(int i) const {
        return Vec2d(_mtx[i][0], _mtx[i][1]);
    }

    /// Gets a column of the matrix as a Vec2.
    Vec2d GetColumn(int i) const {
        return Vec2d(_mtx[0][i], _mtx[1][i]);
    }
    ...
    Matrix2d& Set(double m00, double m01, 
                    double m10, double m11) {
    ...
    Matrix2d& Set(const double m[2][2]) {
        _mtx[0][0] = m[0][0];
        _mtx[0][1] = m[0][1];
        _mtx[1][0] = m[1][0];
        _mtx[1][1] = m[1][1];
    ...
    GF_API
    Matrix2d& SetDiagonal(const Vec2d&);
    ...
    /// Hash.
    friend inline size_t hash_value(Matrix2d const &m) {
    ...
    GF_API
    Matrix2d GetInverse(double* det = NULL, double eps = 0) const;
    ...
    friend Matrix2d operator *(const Matrix2d& m1, double d)
    {
        Matrix2d m = m1;
        return m *= d;
    }
    ...
    ///
    // Returns the product of a matrix and a double.
    friend Matrix2d operator *(double d, const Matrix2d& m)
    {
        return m * d;
    }
    ...
    friend inline Vec2d operator *(const Matrix2d& m, const Vec2d& vec) {
        return Vec2d(vec[0] * m._mtx[0][0] + vec[1] * m._mtx[0][1],
                       vec[0] * m._mtx[1][0] + vec[1] * m._mtx[1][1]);
    }

    /// Returns the product of row vector \e vec and a matrix \e m.
    friend inline Vec2d operator *(const Vec2d &vec, const Matrix2d& m) {
        return Vec2d(vec[0] * m._mtx[0][0] + vec[1] * m._mtx[1][0],
...
private:
    ...
    MatrixData<double, 2, 2> _mtx;

    // Friend declarations
    friend class Matrix2f;
};
...
GF_API 
bool IsClose(Matrix2d const &m1, Matrix2d const &m2, double tolerance);

/// Output a Matrix2d
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream& operator<<(std::ostream &, Matrix2d const &);
