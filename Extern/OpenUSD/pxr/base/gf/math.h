...
///                     be in the range 0-1.
GF_API
double SmoothRamp(double tmin, double tmax, double t, double w0, double w1); 

/// Returns the inner product of \c x with itself: specifically, \c x*x.
/// Defined for \c int, \c float, \c double, and all \c Vec types.
/// \ingroup group_gf_BasicMath
template <class T>
...
/// Return abs(\p f).
/// \ingroup group_gf_BasicMath
inline double Abs(double f) { return std::fabs(f); }
/// Return abs(\p f).
/// \ingroup group_gf_BasicMath
...
/// Return round(\p f).
/// \ingroup group_gf_BasicMath
inline double Round(double f) { return std::rint(f); }
/// Return round(\p f).
/// \ingroup group_gf_BasicMath
inline float Round(float f) { return std::rint(f); }

/// Return pow(\p f, \p p).
/// \ingroup group_gf_BasicMath
inline double Pow(double f, double p) { return std::pow(f, p); }
/// Return pow(\p f, \p p).
/// \ingroup group_gf_BasicMath
...
/// \ingroup group_gf_BasicMath
inline void SinCos(double v, double *s, double *c) { ArchSinCos(v, s, c); }
/// Return sin(\p v) in \p s and cos(\p v) in \p c.
/// \ingroup group_gf_BasicMath
inline void SinCos(float v, float *s, float *c) { ArchSinCosf(v, s, c); }

/// Return the resulting of clamping \p value to lie between
/// \p min and \p max. This function is also defined for Vecs.
/// \ingroup group_gf_BasicMath
...
/// \ingroup group_gf_BasicMath
template <class T>
inline T Lerp( double alpha, const T& a, const T& b) {
...
template <typename Left, typename Right,
          std::enable_if_t<IsArithmetic<Left>::value &&
                           IsArithmetic<Right>::value, int> = 0>
inline decltype(std::declval<Left>() * std::declval<Right>())
Dot(Left left, Right right) {
    return left * right;
}
...
template <typename Left, typename Right,
          std::enable_if_t<IsArithmetic<Left>::value &&
                           IsArithmetic<Right>::value, int> = 0>
inline decltype(std::declval<Left>() * std::declval<Right>())
CompMult(Left left, Right right) {
    return left * right;
}
