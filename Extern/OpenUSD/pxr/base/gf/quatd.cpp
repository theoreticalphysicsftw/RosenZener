...
Quatd::Quatd(Quatf const &other)
    : _imaginary(other.GetImaginary())
    , _real(other.GetReal())
{
}
Quatd::Quatd(Quath const &other)
    : _imaginary(other.GetImaginary())
    , _real(other.GetReal())
{
}
...
std::ostream& 
operator<<(std::ostream &out, Quatd const &q)
{
    Vec3d i = q.GetImaginary();
    double real = q.GetReal();
    return out << Vec4d(real, i[0], i[1], i[2]);
}

double
Quatd::Normalize(double eps)
{
...
//
//     (Q * Quatd(0, P) * Q.GetInverse()).GetImaginary()
...
//
Vec3d
Quatd::Transform(const Vec3d& point) const
{
    double tmpDot = Dot(_imaginary, _imaginary);
