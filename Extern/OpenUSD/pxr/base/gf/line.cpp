...
Vec3d
Line::FindClosestPoint(const Vec3d &point, double *t) const
{
...
bool
FindClosestPoints( const Line &l1, const Line &l2,
                     Vec3d *closest1, Vec3d *closest2,
                     double *t1, double *t2 )
{
    // Define terms:
    ...
    const Vec3d &p1 = l1._p0; 
    const Vec3d &d1 = l1._dir;
    const Vec3d &p2 = l2._p0;
    const Vec3d &d2 = l2._dir;
...
operator<<(std::ostream &out, const Line &line)
{
    return out 
        << '(' 
        << "point:" << _OstreamHelperP(line.GetPoint(0.0)) 
        << ' ' 
        << "direction:" << _OstreamHelperP(line.GetDirection()) 
        << ')';
}
