...
Quatf::Quatf(Quatd const &other)
    : _imaginary(other.GetImaginary())
    , _real(other.GetReal())
{
}
Quatf::Quatf(Quath const &other)
    : _imaginary(other.GetImaginary())
    , _real(other.GetReal())
{
}
...
std::ostream& 
operator<<(std::ostream &out, Quatf const &q)
{
    Vec3f i = q.GetImaginary();
    float real = q.GetReal();
    return out << Vec4f(real, i[0], i[1], i[2]);
}

float
Quatf::Normalize(float eps)
{
...
//
//     (Q * Quatf(0, P) * Q.GetInverse()).GetImaginary()
...
//
Vec3f
Quatf::Transform(const Vec3f& point) const
{
    float tmpDot = Dot(_imaginary, _imaginary);
    float tmpSqr = _real * _real;
    return (2 * Dot(_imaginary, point) * _imaginary +
            (tmpSqr - tmpDot) * point +
            2 * _real * Cross(_imaginary, point)) / (tmpSqr + tmpDot);
}
...
Quatf::operator *=(const Quatf &q)
{
    float r1 = GetReal();
    float r2 = q.GetReal();
    const Vec3f &i1 = GetImaginary();
    const Vec3f &i2 = q.GetImaginary();
