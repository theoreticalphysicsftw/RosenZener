...
public:
    ...
    Rect2i(): _min(0,0), _max(-1,-1)
    {
    }

    /// Constructs a rectangle with \p min and \p max corners.
    Rect2i(const Vec2i& min, const Vec2i& max)
        : _min(min), _max(max)
    {
    }
    ...
    /// and \p height.
    Rect2i(const Vec2i& min, int width, int height)
        : _min(min), _max(min + Vec2i(width-1, height-1))
    {
    }
    ...
    void Translate(const Vec2i& displacement) {
        _min += displacement;
        _max += displacement;
    }
    ...
    Rect2i GetIntersection(const Rect2i& that) const {
        if(IsEmpty())
            return *this;
        else if(that.IsEmpty())
            return that;
        else
            return Rect2i(Vec2i(Max(_min[0], that._min[0]),
                                    Max(_min[1], that._min[1])),
                            Vec2i(Min(_max[0], that._max[0]),
                                    Min(_max[1], that._max[1])));
    }

    /// Computes the intersection of two rectangles.
    /// \deprecated Use GetIntersection() instead
    Rect2i Intersect(const Rect2i& that) const {
        return GetIntersection(that);
    }

    /// Computes the union of two rectangles.
    Rect2i GetUnion(const Rect2i& that) const {
        if(IsEmpty())
            return that;
        else if(that.IsEmpty())
            return *this;
        else
            return Rect2i(Vec2i(Min(_min[0], that._min[0]),
                                    Min(_min[1], that._min[1])),
                            Vec2i(Max(_max[0], that._max[0]),
                                    Max(_max[1], that._max[1])));
    }

    /// Computes the union of two rectangles
    /// \deprecated Use GetUnion() instead.
    Rect2i Union(const Rect2i& that) const {
    ...
    friend inline size_t hash_value(const Rect2i &r) {
        return TfHash::Combine(r._min, r._max);
    }        

    /// Returns true if \p r1 and \p r2 are equal.
    friend bool operator==(const Rect2i& r1, const Rect2i& r2) {
    ...
    /// Computes the union of two rectangles.
    /// \see GetUnion()
    Rect2i operator += (const Rect2i& that) {
        *this = GetUnion(that);
        return *this;
    }

    friend Rect2i operator + (const Rect2i r1, const Rect2i& r2) {
        Rect2i tmp(r1);
        tmp += r2;
        return tmp;
    }
...
/// Output a Rect2i using the format [(x y):(x y)].
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream& operator<<(std::ostream&, const Rect2i&);
