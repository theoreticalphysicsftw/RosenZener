...
template <>
struct IsMatrix<class Matrix4f> { static const bool value = true; };
...
///
/// The following methods interpret a Matrix4f as a 3D
...
public:
    ...
    explicit Matrix4f(const Vec4f& v) {
        SetDiagonal(v);
    }
    ...
    ///
    GF_API
    explicit Matrix4f(const std::vector< std::vector<double> >& v);
    ...
    ///
    GF_API
    explicit Matrix4f(const std::vector< std::vector<float> >& v);
    ...
    ///
    GF_API
    explicit Matrix4f(const std::vector<double>& r0,
    ...
    ///
    GF_API
    explicit Matrix4f(const std::vector<float>& r0,
    ...
    GF_API
    Matrix4f(const Rotation& rotate,
               const Vec3f& translate);

    /// Constructor. Initializes a transformation matrix to perform the
    /// indicated rotation and translation.
    GF_API
    Matrix4f(const Matrix3f& rotmx,
               const Vec3f& translate);
    /// This explicit constructor converts a "double" matrix to a "float" matrix.
    GF_API
    explicit Matrix4f(const class Matrix4d& m);
    ...
    Vec4f GetRow(int i) const {
        return Vec4f(_mtx[i][0], _mtx[i][1], _mtx[i][2], _mtx[i][3]);
    }

    /// Gets a column of the matrix as a Vec4.
    Vec4f GetColumn(int i) const {
        return Vec4f(_mtx[0][i], _mtx[1][i], _mtx[2][i], _mtx[3][i]);
    }
    ...
    Matrix4f& Set(const float m[4][4]) {
        _mtx[0][0] = m[0][0];
        _mtx[0][1] = m[0][1];
        _mtx[0][2] = m[0][2];
        _mtx[0][3] = m[0][3];
    ...
    GF_API
    Matrix4f& SetDiagonal(float s);

    /// Sets the matrix to have diagonal (<c>v[0], v[1], v[2], v[3]</c>).
    GF_API
    Matrix4f& SetDiagonal(const Vec4f&);
    ...
    /// Hash.
    friend inline size_t hash_value(Matrix4f const &m) {
    ...
    GF_API
    Matrix4f GetInverse(double* det = NULL, double eps = 0) const;
    ...
    void SetRow3(int i, const Vec3f & v) {
        _mtx[i][0] = v[0];
        _mtx[i][1] = v[1];
        _mtx[i][2] = v[2];
    }
    ...
    Vec3f GetRow3(int i) const {
        return Vec3f(_mtx[i][0], _mtx[i][1], _mtx[i][2]);
    }
    ...
    bool HasOrthogonalRows3() const {
        ...
        Vec3f axis0(GetRow3(0)), axis1(GetRow3(1)), axis2(GetRow3(2));
    ...
    GF_API
    Matrix4f GetOrthonormalized(bool issueWarning=true) const;
    ...
    friend Matrix4f operator *(const Matrix4f& m1, double d)
    {
        Matrix4f m = m1;
        return m *= d;
    }
    ...
    ///
    // Returns the product of a matrix and a float.
    friend Matrix4f operator *(double d, const Matrix4f& m)
    {
        return m * d;
    }
    ...
    GF_API
    friend Matrix4f operator -(const Matrix4f& m);
    ...
    friend inline Vec4f operator *(const Matrix4f& m, const Vec4f& vec) {
        return Vec4f(vec[0] * m._mtx[0][0] + vec[1] * m._mtx[0][1] + vec[2] * m._mtx[0][2] + vec[3] * m._mtx[0][3],
    ...
    friend inline Vec4f operator *(const Vec4f &vec, const Matrix4f& m) {
        return Vec4f(vec[0] * m._mtx[0][0] + vec[1] * m._mtx[1][0] + vec[2] * m._mtx[2][0] + vec[3] * m._mtx[3][0],
    ...
    GF_API
    Matrix4f& SetScale(float scaleFactor);
    ...
    GF_API
    Matrix4f RemoveScaleShear() const;
    ...
    GF_API
    Matrix4f& SetRotate(const Quatf &rot);

    /// Sets the matrix to specify a rotation equivalent to \e rot,
    /// without clearing the translation.
    GF_API
    Matrix4f& SetRotateOnly(const Quatf &rot);

    /// Sets the matrix to specify a rotation equivalent to \e rot,
    /// and clears the translation.
    GF_API
    Matrix4f& SetRotate(const Rotation &rot);

    /// Sets the matrix to specify a rotation equivalent to \e rot,
    /// without clearing the translation.
    GF_API
    Matrix4f& SetRotateOnly(const Rotation &rot);

    /// Sets the matrix to specify a rotation equivalent to \e mx,
    /// and clears the translation.
    GF_API
    Matrix4f& SetRotate(const Matrix3f &mx);

    /// Sets the matrix to specify a rotation equivalent to \e mx,
    /// without clearing the translation.
    GF_API
    Matrix4f& SetRotateOnly(const Matrix3f &mx);

    /// Sets the matrix to specify a nonuniform scaling in x, y, and z by
    /// the factors in vector \e scaleFactors.
    GF_API
    Matrix4f& SetScale(const Vec3f &scaleFactors);

    /// Sets matrix to specify a translation by the vector \e trans,
    /// and clears the rotation.
    GF_API
    Matrix4f& SetTranslate(const Vec3f &trans);

    /// Sets matrix to specify a translation by the vector \e trans,
    /// without clearing the rotation.
    GF_API
    Matrix4f& SetTranslateOnly(const Vec3f &t);

    /// Sets matrix to specify a rotation by \e rotate and a
    /// translation by \e translate.
    GF_API
    Matrix4f& SetTransform(const Rotation& rotate,
                             const Vec3f& translate);

    /// Sets matrix to specify a rotation by \e rotmx and a
    /// translation by \e translate.
    GF_API
    Matrix4f& SetTransform(const Matrix3f& rotmx,
                             const Vec3f& translate);
    ...
    GF_API
    Matrix4f& SetLookAt(const Vec3f &eyePoint,
    ...
    GF_API
    bool Factor(Matrix4f* r, Vec3f* s, Matrix4f* u,
                Vec3f* t, Matrix4f* p,
    ...
    Vec3f ExtractTranslation() const {
        return Vec3f(_mtx[3][0], _mtx[3][1], _mtx[3][2]);
    }
    ...
    /// this method.
    GF_API
    Quatf  ExtractRotationQuat() const;
    ...
    GF_API
    Vec3f DecomposeRotation(const Vec3f &axis0,
                              const Vec3f &axis1,
    ...
    /// this method.
    GF_API
    Matrix3f ExtractRotationMatrix() const;
    ...
    Vec3f TransformDir(const Vec3f &vec) const {
        return Vec3f(
            vec[0] * _mtx[0][0] + vec[1] * _mtx[1][0] + vec[2] * _mtx[2][0],
            vec[0] * _mtx[0][1] + vec[1] * _mtx[1][1] + vec[2] * _mtx[2][1],
            vec[0] * _mtx[0][2] + vec[1] * _mtx[1][2] + vec[2] * _mtx[2][2]);
    ...
    /// is 1 and ignores the fourth column of the matrix (i.e. assumes it is
    /// (0, 0, 0, 1)).
    Vec3f TransformAffine(const Vec3f &vec) const {
        return Vec3f(
            vec[0] * _mtx[0][0] + vec[1] * _mtx[1][0] + vec[2] * _mtx[2][0] + _mtx[3][0],
            vec[0] * _mtx[0][1] + vec[1] * _mtx[1][1] + vec[2] * _mtx[2][1] + _mtx[3][1],
            vec[0] * _mtx[0][2] + vec[1] * _mtx[1][2] + vec[2] * _mtx[2][2] + _mtx[3][2]);
...
private:
    ...
    void _SetRotateFromQuat(float r, const Vec3f& i);


private:
    /// Matrix storage, in row-major order.
    MatrixData<float, 4, 4> _mtx;
...
GF_API 
bool IsClose(Matrix4f const &m1, Matrix4f const &m2, double tolerance);

/// Output a Matrix4f
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream& operator<<(std::ostream &, Matrix4f const &);
