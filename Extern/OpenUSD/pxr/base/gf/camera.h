...
public:
    GF_API Camera(
        const Matrix4d &transform = Matrix4d(1.0),
        ...
        const Range1f &clippingRange = Range1f(1, 1000000),
        const std::vector<Vec4f> &clippingPlanes = std::vector<Vec4f>(),
        float fStop = 0.0,
        float focusDistance = 0.0);

    /// Sets the transform of the filmback in world space to \p val. 
    GF_API void SetTransform(const Matrix4d &val);
    ...
    GF_API void SetFromViewAndProjectionMatrix(
        const Matrix4d &viewMatrix, const Matrix4d &projMatix,
        const float focalLength = 50.0);

    /// @}

    /// Sets the clipping range in world units.
    GF_API void SetClippingRange(const Range1f &val);
    ...
    GF_API void SetClippingPlanes(const std::vector<Vec4f> &val);
    ...
    GF_API Matrix4d GetTransform() const;
    ...
    GF_API bool operator==(const Camera& other) const;
