...
template <>
struct IsMatrix<class Matrix3f> { static const bool value = true; };
...
/// a Matrix3f as a 3D transformation. By convention, vectors are treated
...
public:
    ...
    ///
    GF_API
    explicit Matrix3f(const std::vector< std::vector<double> >& v);
    ...
    void SetColumn(int i, const Vec3f & v) {
        _mtx[0][i] = v[0];
        _mtx[1][i] = v[1];
        _mtx[2][i] = v[2];
    }
    ...
    Vec3f GetColumn(int i) const {
        return Vec3f(_mtx[0][i], _mtx[1][i], _mtx[2][i]);
    }
    ...
    Matrix3f& Set(float m00, float m01, float m02, 
                    float m10, float m11, float m12, 
                    float m20, float m21, float m22) {
    ...
    Matrix3f& Set(const float m[3][3]) {
        _mtx[0][0] = m[0][0];
        _mtx[0][1] = m[0][1];
        _mtx[0][2] = m[0][2];
        _mtx[1][0] = m[1][0];
    ...
    GF_API
    Matrix3f& SetDiagonal(const Vec3f&);
    ...
    /// Hash.
    friend inline size_t hash_value(Matrix3f const &m) {
    ...
    GF_API
    Matrix3f GetInverse(double* det = NULL, double eps = 0) const;
    ...
    GF_API
    Matrix3f GetOrthonormalized(bool issueWarning=true) const;
    ...
    GF_API
    Matrix3f& operator *=(double);

    /// Returns the product of a matrix and a float.
    friend Matrix3f operator *(const Matrix3f& m1, double d)
    {
        Matrix3f m = m1;
        return m *= d;
    }
    ...
    ///
    // Returns the product of a matrix and a float.
    friend Matrix3f operator *(double d, const Matrix3f& m)
    {
        return m * d;
    }
    ...
    GF_API
    friend Matrix3f operator -(const Matrix3f& m);
    ...
    friend inline Vec3f operator *(const Matrix3f& m, const Vec3f& vec) {
        return Vec3f(vec[0] * m._mtx[0][0] + vec[1] * m._mtx[0][1] + vec[2] * m._mtx[0][2],
    ...
    friend inline Vec3f operator *(const Vec3f &vec, const Matrix3f& m) {
        return Vec3f(vec[0] * m._mtx[0][0] + vec[1] * m._mtx[1][0] + vec[2] * m._mtx[2][0],
    ...
    GF_API
    Matrix3f& SetScale(float scaleFactor);
    ...
    GF_API
    Matrix3f& SetScale(const Vec3f &scaleFactors);
...
GF_API 
bool IsClose(Matrix3f const &m1, Matrix3f const &m2, double tolerance);

/// Output a Matrix3f
/// \ingroup group_gf_DebuggingOutput
GF_API std::ostream& operator<<(std::ostream &, Matrix3f const &);
