...
void
Plane::Set(const Vec3d &normal, const Vec3d &point)
{
    _normal = normal.GetNormalized();
    _distance = Dot(_normal, point);
}

void
Plane::Set(const Vec3d &p0, const Vec3d &p1, const Vec3d &p2)
{
    _normal = Cross(p1 - p0, p2 - p0).GetNormalized();
    _distance = Dot(_normal, p0);
}

void
Plane::Set(const Vec4d &eqn)
{
...
Vec4d
Plane::GetEquation() const
 {
     return Vec4d(_normal[0], _normal[1], _normal[2], -_distance);
 }

Plane &
Plane::Transform(const Matrix4d &matrix) 
{
    ...
    // incorrectly.
    const Matrix4d adjoint = matrix.GetInverse().GetTranspose();
...
bool
Plane::IntersectsPositiveHalfSpace(const Range3d &box) const
{
    if (box.IsEmpty())
	return false;
...
bool
FitPlaneToPoints(const std::vector<Vec3d>& points, Plane* fitPlane)
{
    ...
    Vec3d sumOfPoints(0.0);
    for (const Vec3d& p : points) {
        sumOfPoints += p;
    }
    const Vec3d centroid = sumOfPoints / points.size();
    ...
    const double det1 = Abs(ata1.GetDeterminant());
    const double det2 = Abs(ata2.GetDeterminant());
    const double det3 = Abs(ata3.GetDeterminant());
    Vec3d equation;
    if (det1 > 0.0 && det1 > det2 && det1 > det3) {
        // A^T B = {{\sum (y_i) (-x_i)},
        //          {\sum (z_i) (-x_i)}}
        // X = {{b}, {c}}
        const Vec2d atb1(-xy, -xz);
        const Vec2d leastSquaresEstimate = ata1.GetInverse() * atb1;
        equation[0] = 1.0;
        ...
        //          {\sum (z_i) (-y_i)}}
        // X = {{a}, {c}}
        const Vec2d atb2(-xy, -yz);
        const Vec2d leastSquaresEstimate = ata2.GetInverse() * atb2;
        ...
        //          {\sum (y_i) (z_i)}}
        // X = {{a}, {b}}
        const Vec2d atb3(-xz, -yz);
        const Vec2d leastSquaresEstimate = ata3.GetInverse() * atb3;
        equation[0] = leastSquaresEstimate[0];
    ...
    //                        => (d = -{a, b, c} . {x, y, z})
    const double d = -Dot(equation, centroid);
    fitPlane->Set(Vec4d(equation[0], equation[1], equation[2], d));
    return true;
}

std::ostream &
operator<<(std::ostream& out, const Plane& p)
{
    return out
        << '[' << _OstreamHelperP(p.GetNormal()) << " " 
        << _OstreamHelperP(p.GetDistanceFromOrigin()) << ']';
}
